...
import { uploadEventImage } from '@/features/events/upload';
...
export function EventImageUploader({ value, onChange, onClear, error }: EventImageUploaderProps) {
  ...
  const handleFile = useCallback(
    async (file: File) => {
      ...
      setProgress(0);
      try {
        const url = await uploadEventImage(file, setProgress);
        onChange(url);
      } catch (err) {
